...
import{ QuizDataSchema, TheoryQuizDataSchema, type Question, type QuizType, type TheoryQuestion } from '@/types'; 
...
export async function generateQuizPDFWithPdfMake(
  quizData: (Question | TheoryQuestion)[],
  quizTitle: string,
  quizType: QuizType,
  quizTags?: string[]
): Promise<Uint8Array> {
  ...
  const docDefinition: TDocumentDefinitions = {
    ...
    footer: function(currentPage, pageCount) {
      return {
        columns: [
          { text: `Page ${currentPage} of ${pageCount}`, alignment: 'left', margin: [40, 10, 0, 20] as [number, number, number, number], fontSize: 9 },
          { text: 'Generated by Xyen AI', alignment: 'right', margin: [0, 10, 40, 20] as [number, number, number, number], fontSize: 9, italics: true }
        ],
      };
    },
    content: [
