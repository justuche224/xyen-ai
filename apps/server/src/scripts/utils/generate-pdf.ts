...
async function uploadPDFToSupabase(pdfBuffer: Buffer, userId: string, quizTitle: string): Promise<string> {
...
export const generateQuizPDFHandler = protectedProcedure
  ...
  .handler(async ({ input }) => {
    try {
      ...
      const data= quizData.data as (Question | TheoryQuestion)[];
      
      const pdfUint8Array = await generateQuizPDFWithPdfMake(
        data,
        quizData.title,
        quizData.quizType,
        quizData.tags ?? undefined
      );
