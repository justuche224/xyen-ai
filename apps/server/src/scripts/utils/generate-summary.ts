...
interface GenerateSummaryOptions {
  ...
  outputPercentage?: number; // e.g., 30 for 30%
  ...
  try {
    ...
    if (!text || text.trim() === "") {
      ...
      if (response.promptFeedback?.blockReason) {
        throw new Error(
          `AI request blocked: ${response.promptFeedback.blockReason} - ${response.promptFeedback.blockReasonMessage}`
        );
      }
      throw new Error("AI returned an empty summary.");
    }

    function cleanMarkdownCodeBlock(text: string) {
