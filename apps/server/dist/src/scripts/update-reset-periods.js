    ...
    try {
        for (const update of updates) {
            const result = await db
                ...
                .set({
                resetPeriod: update.resetPeriod,
                updatedAt: new Date(),
            })
                .where(and(eq(featureLimits.planType, update.planType), eq(featureLimits.featureKey, update.featureKey)));
            console.log(`✅ Updated ${update.planType} - ${update.featureKey} to reset ${update.resetPeriod}`);
        }
