...
async function testFeatureLimits() {
    ...
    try {
        ...
        let check1 = await FeatureLimitService.canUseFeature(testUserId, "daily_generations", 1);
        console.log(`First generation attempt: ${check1.allowed ? "✅ ALLOWED" : "❌ DENIED"} (${check1.current}/3)`);
        if (check1.allowed) {
            await FeatureLimitService.trackUsage(testUserId, "daily_generations", 1);
        }
        // Track 2 more generations
        for (let i = 2; i <= 3; i++) {
            let check = await FeatureLimitService.canUseFeature(testUserId, "daily_generations", 1);
            console.log(`Generation ${i} attempt: ${check.allowed ? "✅ ALLOWED" : "❌ DENIED"} (${check.current}/3)`);
            if (check.allowed) {
                await FeatureLimitService.trackUsage(testUserId, "daily_generations", 1);
            }
        }
        // Try 4th generation - should be denied
        let check4 = await FeatureLimitService.canUseFeature(testUserId, "daily_generations", 1);
        console.log(`Fourth generation attempt: ${check4.allowed ? "✅ ALLOWED" : "❌ DENIED"} (${check4.current}/3)`);
        ...
        let pdfCheck1 = await FeatureLimitService.canUseFeature(testUserId, "pdf_exports", 1);
        console.log(`First PDF export: ${pdfCheck1.allowed ? "✅ ALLOWED" : "❌ DENIED"} (${pdfCheck1.current}/3)`);
        ...
        let questionCheck = await FeatureLimitService.canUseFeature(testUserId, "max_questions", 10);
        console.log(`10 questions: ${questionCheck.allowed ? "✅ ALLOWED" : "❌ DENIED"}`);
        let questionCheck15 = await FeatureLimitService.canUseFeature(testUserId, "max_questions", 15);
        console.log(`15 questions: ${questionCheck15.allowed ? "✅ ALLOWED" : "❌ DENIED"}`);
        ...
        // Test pro limits
        let proCheck = await FeatureLimitService.canUseFeature(testUserId, "daily_generations", 1);
        console.log(`Pro daily generation: ${proCheck.allowed ? "✅ ALLOWED (unlimited)" : "❌ DENIED"}`);
        let proQuestionCheck = await FeatureLimitService.canUseFeature(testUserId, "max_questions", 25);
        console.log(`Pro 25 questions: ${proQuestionCheck.allowed ? "✅ ALLOWED" : "❌ DENIED"}`);
