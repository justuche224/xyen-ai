...
async function generateQuiz(documentLink, quiztype, difficulty, questionCount, customePrompt) {
    try {
        const document = await fetch(documentLink).then((response) => response.arrayBuffer());
        const contents = [
        ...
        const response = await ai.models.generateContent({
            ...
            contents: contents,
            config: {
                systemInstruction: `You are an expert quiz generator. Your task is to output a quiz in valid JSON format only—do not include any extra text or commentary. Generate a quiz with ${questionCount ?? 10} ${quiztype === "multiple-choice"
                    ? "multiple choice"
                    : quiztype === "yes-no"
                        ? "yes or no"
                        : "theory"} questions. The JSON should follow this structure:
        JSON:
        [
