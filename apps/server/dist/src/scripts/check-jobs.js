...
async function checkJobsAndUsage() {
    ...
    try {
        ...
        if (processingJob) {
            ...
            const dailyCheck = await FeatureLimitService.canUseFeature(userId, "daily_generations", 1);
            console.log("Daily generations check:", dailyCheck);
            const concurrentCheck = await FeatureLimitService.canUseFeature(userId, "concurrent_jobs", 1);
            console.log("Concurrent jobs check:", concurrentCheck);
            const questionCheck = await FeatureLimitService.canUseFeature(userId, "max_questions", 10);
