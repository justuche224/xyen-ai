...
export const quizRouter = {
    ...
    getQuizById: protectedProcedure
        ...
        const attemptStats = await db
            .select({
            totalAttempts: sql `count(${quizAttempts.id})`.as("totalAttempts"),
            averageScore: sql `avg(${quizAttempts.score})`.as("averageScore"),
            totalTimeSpent: sql `sum(extract(epoch from (${quizAttempts.endTime} - ${quizAttempts.startTime})))`.as("totalTimeSpent"),
            averageTimeSpent: sql `avg(extract(epoch from (${quizAttempts.endTime} - ${quizAttempts.startTime})))`.as("averageTimeSpent"),
        })
            .from(quizAttempts)
    ...
    getAttempt: protectedProcedure
        ...
        const result = await db
            .select()
            .from(quizAttempts)
            .where(and(eq(quizAttempts.id, input.attemptId), eq(quizAttempts.userId, input.userId)))
            .limit(1);
    ...
    getUserAttempts: protectedProcedure
        ...
        return await db
            ...
            quizType: quiz.quizType,
            duration: sql `extract(epoch from (${quizAttempts.endTime} - ${quizAttempts.startTime}))`.as("duration"),
        })
            .from(quizAttempts)
    ...
    getQuizStats: protectedProcedure
        ...
        const stats = await db
            .select({
            totalAttempts: sql `count(*)`.as("totalAttempts"),
            completedAttempts: sql `count(*) filter (where ${quizAttempts.status} = 'completed')`.as("completedAttempts"),
            averageScore: sql `avg(${quizAttempts.score}) filter (where ${quizAttempts.status} = 'completed')`.as("averageScore"),
            highestScore: sql `max(${quizAttempts.score}) filter (where ${quizAttempts.status} = 'completed')`.as("highestScore"),
            lowestScore: sql `min(${quizAttempts.score}) filter (where ${quizAttempts.status} = 'completed')`.as("lowestScore"),
            averageDuration: sql `avg(extract(epoch from (${quizAttempts.endTime} - ${quizAttempts.startTime}))) filter (where ${quizAttempts.status} = 'completed')`.as("averageDuration"),
            practiceAttempts: sql `count(*) filter (where ${quizAttempts.mode} = 'practice')`.as("practiceAttempts"),
            examAttempts: sql `count(*) filter (where ${quizAttempts.mode} = 'exam')`.as("examAttempts"),
            reviewAttempts: sql `count(*) filter (where ${quizAttempts.mode} = 'review')`.as("reviewAttempts"),
        })
            .from(quizAttempts)
            .where(eq(quizAttempts.quizId, input.quizId));
    ...
    getUserQuizStats: protectedProcedure
        ...
        const [basicStats] = await db
            .select({
            totalQuizzes: sql `count(distinct ${quiz.id})`.as("totalQuizzes"),
            totalAttempts: sql `count(${quizAttempts.id})`.as("totalAttempts"),
            completedAttempts: sql `count(*) filter (where ${quizAttempts.status} = 'completed')`.as("completedAttempts"),
            averageScore: sql `avg(${quizAttempts.score}) filter (where ${quizAttempts.status} = 'completed')`.as("averageScore"),
            totalTimeSpent: sql `sum(extract(epoch from (${quizAttempts.endTime} - ${quizAttempts.startTime}))) filter (where ${quizAttempts.status} = 'completed')`.as("totalTimeSpent"),
        })
            .from(quiz)
        ...
        const bestScore = await db
            .select({
            bestScore: sql `max(${quizAttempts.score}) filter (where ${quizAttempts.status} = 'completed')`.as("bestScore"),
        })
            .from(quizAttempts)
        ...
        const difficultyStats = await db
            .select({
            difficulty: quizMeta.difficulty,
            count: sql `count(distinct ${quiz.id})`.as("count"),
        })
            .from(quiz)
            .leftJoin(quizMeta, eq(quiz.id, quizMeta.quizId))
            .where(eq(quiz.userId, input.userId))
        ...
        const byDifficulty = Object.fromEntries(difficultyStats.map((stat) => [stat.difficulty, stat.count]));
        const byQuizType = Object.fromEntries(quizTypeStats.map((stat) => [stat.quizType, stat.count]));
        const stats = {
            ...basicStats,
            byDifficulty,
            byQuizType,
    ...
    bulkDeleteAttempts: protectedProcedure
        ...
        const result = await db
            .delete(quizAttempts)
            .where(and(sql `${quizAttempts.id} = ANY(${input.attemptIds})`, eq(quizAttempts.userId, input.userId)));
