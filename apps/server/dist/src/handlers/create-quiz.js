...
export const createQuizHandler = protectedProcedure
    ...
    const questionCheck = await FeatureLimitService.canUseFeature(input.userId, "max_questions", input.questionCount);
    ...
    const jobCheck = await FeatureLimitService.canUseFeature(input.userId, "concurrent_jobs");
    if (!jobCheck.allowed) {
        throw new ORPCError("FORBIDDEN", {
    ...
    try {
        ...
        await FeatureLimitService.trackUsage(input.userId, "daily_generations", 1, {
            questionCount: input.questionCount,
            quizType: input.quizType,
        });
        return {
