...
export const getAllUsersHandler = adminProcedure
    ...
    .handler(async ({ input }) => {
    console.log(input);
    const { page, limit, search, role, emailVerified, sortBy, sortOrder } = input;
    ...
    if (whereConditions.length > 0) {
        totalCountQuery.where(sql `${whereConditions.reduce((acc, condition, i) => i === 0 ? condition : sql `${acc} AND ${condition}`)}`);
    }
    ...
    let query = db
        ...
        .from(user)
        .leftJoin(userPlans, and(eq(user.id, userPlans.userId), eq(userPlans.status, "active")))
        .orderBy(orderBy)
        .limit(limit)
        .offset(offset);
    if (whereConditions.length > 0) {
        // @ts-ignore
        query = query.where(sql `${whereConditions.reduce((acc, condition, i) => i === 0 ? condition : sql `${acc} AND ${condition}`)}`);
    }
...
export const getUserStatsHandler = adminProcedure.handler(async () => {
    const stats = await db
        ...
        verifiedUsers: count(sql `CASE WHEN ${user.emailVerified} = true THEN 1 END`),
        unverifiedUsers: count(sql `CASE WHEN ${user.emailVerified} = false THEN 1 END`),
    })
        .from(user);
    return stats[0];
});
